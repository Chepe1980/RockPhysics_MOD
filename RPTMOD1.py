...
def main():
        ...
        if st.button("Run Modeling", type="primary"):
            if modeling_type == "Single Point":
                ...
                if results_df is not None:
                    ...
                    if 'Vp_measured' in results_df.columns:
                        fig_depth.add_trace(
                            go.Scatter(x=results_df['Vp_measured'], y=results_df['Depth'], 
                                      mode='lines', name='Measured Vp', line=dict(color='blue')),
                            row=1, col=1
                        )
                    fig_depth.add_trace(
                        go.Scatter(x=results_df['Vp_modeled'], y=results_df['Depth'], 
                                  mode='lines', name='Modeled Vp', line=dict(color='red', dash='dash')),
                        row=1, col=1
                    )
                    
                    if 'Vs_measured' in results_df.columns:
                        fig_depth.add_trace(
                            go.Scatter(x=results_df['Vs_measured'], y=results_df['Depth'], 
                                      mode='lines', name='Measured Vs', line=dict(color='green')),
                            row=1, col=2
                        )
                    fig_depth.add_trace(
                        go.Scatter(x=results_df['Vs_modeled'], y=results_df['Depth'], 
                                  mode='lines', name='Modeled Vs', line=dict(color='orange', dash='dash')),
                        row=1, col=2
                    )
                    
                    if 'RHOB_measured' in results_df.columns:
                        fig_depth.add_trace(
                            go.Scatter(x=results_df['RHOB_measured'], y=results_df['Depth'], 
                                      mode='lines', name='Measured RHOB', line=dict(color='purple')),
                            row=1, col=3
                        )
                    fig_depth.add_trace(
                        go.Scatter(x=results_df['RHOB_modeled'], y=results_df['Depth'], 
                                  mode='lines', name='Modeled RHOB', line=dict(color='brown', dash='dash')),
                        row=1, col=3
                    )
                    ...
                    if all(col in results_df.columns for col in ['Vp_measured', 'Vs_measured', 'RHOB_measured']):
                        ...
                        fig_cross = make_subplots(
                            rows=2, cols=2,
                            subplot_titles=['Vp: Measured vs Modeled', 'Vs: Measured vs Modeled', 
                                          'RHOB: Measured vs Modeled', 'Vp/Vs Ratio Comparison'],
                            specs=[[{}, {}], [{}, {}]]
                        )
                        
                        # Vp cross-plot
                        fig_cross.add_trace(
                            go.Scatter(x=results_df['Vp_measured'], y=results_df['Vp_modeled'],
                                      mode='markers', name='Vp',
                                      marker=dict(size=6, color=results_df['Depth'], colorscale='Viridis', showscale=False)),
                            row=1, col=1
                        )
                        ...
                        fig_cross.add_trace(
                            go.Scatter(x=[min_vp, max_vp], y=[min_vp, max_vp],
                        ...
                        # Vs cross-plot
                        fig_cross.add_trace(
                            go.Scatter(x=results_df['Vs_measured'], y=results_df['Vs_modeled'],
                                      mode='markers', name='Vs',
                                      marker=dict(size=6, color=results_df['Depth'], colorscale='Viridis', showscale=False)),
                            row=1, col=2
                        )
                        ...
                        fig_cross.add_trace(
                            go.Scatter(x=[min_vs, max_vs], y=[min_vs, max_vs],
                                      mode='lines', name='1:1 Line', line=dict(color='red', dash='dash')),
                            row=1, col=2
                        )
                        ...
                        # RHOB cross-plot
                        fig_cross.add_trace(
                            go.Scatter(x=results_df['RHOB_measured'], y=results_df['RHOB_modeled'],
                                      mode='markers', name='RHOB',
                                      marker=dict(size=6, color=results_df['Depth'], colorscale='Viridis', showscale=True)),
                            row=2, col=1
                        )
                        ...
                        fig_cross.add_trace(
                            go.Scatter(x=[min_rhob, max_rhob], y=[min_rhob, max_rhob],
                                      mode='lines', name='1:1 Line', line=dict(color='red', dash='dash')),
                            row=2, col=1
                        )
                        ...
                        fig_cross.add_trace(
                            go.Scatter(x=vp_vs_measured, y=vp_vs_modeled,
                                      mode='markers', name='Vp/Vs',
                                      marker=dict(size=6, color=results_df['Depth'], colorscale='Viridis', showscale=False)),
                            row=2, col=2
                        )
                        ...
                        fig_cross.add_trace(
                            go.Scatter(x=[min_vp_vs, max_vp_vs], y=[min_vp_vs, max_vp_vs],
                                      mode='lines', name='1:1 Line', line=dict(color='red', dash='dash')),
                            row=2, col=2
                        )
    ...
    with tab5:
        ...
        def gassmann_fluid_substitution(vp_orig, vs_orig, rhob_orig, phi, sw_orig, sw_new, 
            ...
            """
            Simplified Gassmann fluid substitution
            """
            # Fluid properties
            fluid_props = {

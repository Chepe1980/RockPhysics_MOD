            ...
            if modeling_type == "Single Point":
                ...
                if results_df is not None:
                    ...
                    if all(col in results_df.columns for col in ['Vp_measured', 'Vs_measured', 'RHOB_measured']):
                        ...
                        # Color coding option
                        color_option = 'SW' if 'SW' in results_df.columns else 'Depth'
                        color_data = results_df[color_option] if color_option in results_df.columns else results_df['Depth']
                        ...
                        fig_cross.add_trace(
                            go.Scatter(
                                ...
                                marker=dict(
                                    ...
                                    colorscale='Viridis',
                                    showscale=True,
                                    colorbar=dict(title=color_option, x=0.45, y=0.5)
                                ),
                                name='Vp',
                                hovertemplate=(
                                    f"<b>Measured Vp</b>: %{{x:.0f}} m/s<br>"
                                    f"<b>Modeled Vp</b>: %{{y:.0f}} m/s<br>"
                                    f"<b>{color_option}</b>: %{{marker.color:.3f}}<br>"
                                    f"<b>Depth</b>: {results_df['Depth'].iloc[0]:.1f}<extra></extra>"
                                )
                            ),
                            row=1, col=1
                        )
                        ...
                        fig_cross.update_layout(
                            ...
                            template="plotly_white",
                            showlegend=False,
                            title=f"Rock Physics Modeling Scatter Plots (Color coded by {color_option})"
                        )
